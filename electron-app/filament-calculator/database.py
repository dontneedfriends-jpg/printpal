...
def insert_shpoolken_filaments(filaments_data):
    ...
    for item in filaments_data:
        ...
        for filament in item.get("filaments", []):
            ...
            if name and ("{" in name or "color_name" in name.lower()):
                name = manufacturer + " " + filament.get("material", "")
            ...
            for weight_info in weights:
                weight = safe_float(weight_info.get("weight"), 1000, 1, 50000)
                spool_weight = safe_float(weight_info.get("spool_weight"), 250, 0, 10000)
                
                for color in colors:
                    color_hex = color.get("hex")
                    color_name = color.get("name") or ""
                    if color_name and ("{" in color_name or "color_name" in color_name.lower()):
                        color_name = color_hex or ""
                    if not color_name and color_hex:
                        color_name = "#" + color_hex.upper()
                    if not color_name:
                        color_name = color_hex or ""
                    
                    for diameter in diameters:
                        diameter_val = safe_float(diameter, 1.75, 0.5, 5.0)
                        density_val = safe_float(density, None, 0.01, 10.0) if density else None
                        extruder_val = safe_int(extruder_temp, None, 0, 500) if extruder_temp else None
                        bed_val = safe_int(bed_temp, None, 0, 300) if bed_temp else None
                        ...
                        conn.execute("""
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """, (manufacturer, name, material, color_name, color_hex, density_val, diameter_val, weight, spool_weight, extruder_val, bed_val, finish, pattern, glow, translucent))
